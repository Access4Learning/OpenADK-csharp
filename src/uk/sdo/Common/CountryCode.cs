// THIS FILE WAS AUTO-GENERATED BY ADKGEN -- DO NOT MODIFY!

//
// Copyright (c)1998-2011 Pearson Education, Inc. or its affiliate(s).
// All rights reserved.
//

using System;
using OpenADK.Library;

namespace OpenADK.Library.uk.Common
{
	///<summary>
	/// Defines the set of values that can be specified whenever a CountryCode
	/// is used as a parameter to a method or constructor. 
	///</summary>
	/// <remarks>
	/// Alternatively, the static
	///  <see cref="Wrap"/> method can be called to encapsulate any string value in
	///  a CountryCode object.
	/// <para>Author: Generated by adkgen</para>
	/// <para>Version: 2.5</para>
	/// <para>Since: 2.0</para>
	/// </remarks>
	[Serializable]
	public class CountryCode : SifEnum
	{
	/// <summary>Isle of Man ("IMN")</summary>
	public static readonly CountryCode IMN = new CountryCode("IMN");

	/// <summary>Ireland ("IRL")</summary>
	public static readonly CountryCode IRL = new CountryCode("IRL");

	/// <summary>United Kingdom of Great Britain and Northern Ireland ("GBR")</summary>
	public static readonly CountryCode GBR = new CountryCode("GBR");

	///<summary>Wrap an arbitrary string value in a CountryCode object.</summary>
	///<param name="wrappedValue">The element/attribute value.</param>
	///<remarks>This method does not verify
	///that the value is valid according to the SIF Specification</remarks>
	public static CountryCode Wrap( String wrappedValue ) {
		return new CountryCode( wrappedValue );
	}

	private CountryCode( string enumDefValue ) : base( enumDefValue ) {}
	}
}
